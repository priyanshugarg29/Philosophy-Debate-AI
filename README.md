...
The app reads three philosophy corpuses, builds separate Chroma indexes for each one, and lets each agent debate from its own source material.
...
If you add or change corpus files, rebuild first:
...
5. Add `GROQ_API_KEY` in the app secrets or environment settings.
6. Deploy.

Notes:

- `packages.txt` is included for Tesseract on Streamlit Community Cloud.
- `storage/` is gitignored for generated indexes.
- The first cloud run may take time because the embedding model must download and indexes must build.
...
- my Groq API key
- optional Streamlit Community Cloud secrets
